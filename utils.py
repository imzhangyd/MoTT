...
def find_near(pdcontent, x, y):
    ...
    xidx = pdcontent.columns.get_loc("pos_x")
    yidx = pdcontent.columns.get_loc("pos_y")
    all_posi = pdcontent.values.tolist()
    dis_all_posi = []
    for thisframepos in all_posi:
        dis = (thisframepos[xidx] - x) ** 2 + (thisframepos[yidx] - y) ** 2
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pred_csvpath",
        type=str,
        default="./prediction/20240304_09_44_18/track_result.csv",
    )
    parser.add_argument(
        "--save_xmlpath",
        type=str,
        default="./prediction/20240304_09_44_18/track_result.xml",
    )
