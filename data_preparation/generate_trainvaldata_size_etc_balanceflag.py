        ...
        if len(frampos[0]) > 0:
            all_posi.append(
                [
                    panum,
                    frampos[0].item(),
                    np_parpos[frampos[0].item(), 0],
                    np_parpos[frampos[0].item(), 1],
                ]
            )
...
def make_data(
    csvcontent, SIG, outputfolder, name, Past, Cand, n_near, frameend, add_stop_ratio
):
    ...
    for pa_ind, paticle_poslist in enumerate(csvcontent):
        print(
            "Particle number:{}/{}, with length:{}".format(
                pa_ind, len(csvcontent), len(paticle_poslist)
            )
        )
        if len(paticle_poslist) >= 1 + Cand:
            ...
            for a in range(Past - 1, 0, -1):
                padding_before.append(
                    [-1, -1, first_frame - a] + [-1] * 3 + [p_ind]
                )  # -1*3 -> size intensity z
            padding_after = []
            for b in range(Cand):
            ...
            for line_ind in range(p_poslist_len):  # shifting windows
                    ...
                    for tobe_extlabel in nodenamelist:
                        ...
                        for ppos in np_re:
                            ...
                            if notequalGT:
                                ...
                                if (
                                    csvcontent[rand_i][fra_num][0]
                                    == tree.get_node(GTlabel).data[0]
                                    and csvcontent[rand_i][fra_num][1]
                                    == tree.get_node(GTlabel).data[1]
                                ):
                                    continue
                            numb += 1
                ...
                if len(str_candlist) != len(set(str_candlist)):
                    print(
                        "Skip! Generated candidate duplicates!{}-{}".format(
                            paticle_poslist, line_ind
                        )
                    )
                    continue
                ...
                # ++++++++add_stop tracks+++++++++++++++++++++++++++++++++++++++++++++++++++
                if not line_ind in [0, p_poslist_len]:
                    tree_stop = Tree()
                    tree_stop.create_node(
                        tag="ext", identifier="ext", data=pastposlist[-1]
                    )
                    for j in range(Cand):
                        nodename = "ext" + "1" * (j + 1)
                        tree_stop.create_node(
                    ...
                    for j in range(Cand):
                        ...
                        tree_stop.create_node(
                            ...
                            data=[
                                ...
                                pad_paticle_poslist[line_ind + Past + j][1],  # y
                                pad_paticle_poslist[line_ind + Past + j][3],  # size
                                pad_paticle_poslist[line_ind + Past + j][
                                    4
                                ],  # intensity
                                pad_paticle_poslist[line_ind + Past + j][5],  # z
                            ],
                    ...
                    nodenamelist = ["ext"]
                    for idx, frame_ind__ in enumerate(frame_indnext_list):
                        newnamelist = []
                        for tobe_extlabel in nodenamelist:

                            # confirm the current position
                            thisnodedata = tree_stop.get_node(tobe_extlabel).data
                            if thisnodedata[-1] == -1:
                                parentnodelabel = tobe_extlabel
                                for _ in range(Cand):
                                    parentnodelabel = tree_stop.parent(
                                        parentnodelabel
                                    ).tag
                                    parentnode = tree_stop.get_node(parentnodelabel)
                                    if parentnode.data[-1] != -1:
                                        near_objpos = parentnode.data.copy()
                                        break
                            else:
                            ...
                            # children number of this node
                            if tree_stop.children(tobe_extlabel) == []:  # null
                                numb = 0
                                neednull = 1
                                notequalGT = 1
                            else:  # has a GT children
                                ...
                                numb = 1
                                # Is the GT children real or dumn detection
                                if tree_stop.get_node(nodename).data[-1] == -1:
                                    neednull = 0
                                    notequalGT = 1
                                else:
                                    neednull = 1
                            ...
                            for ppos in np_re:
                                ...
                                if notequalGT:
                                    ...
                                    if (
                                        csvcontent[rand_i][fra_num][0]
                                        == tree_stop.get_node(GTlabel).data[0]
                                        and csvcontent[rand_i][fra_num][1]
                                        == tree_stop.get_node(GTlabel).data[1]
                                    ):
                                        continue
                                numb += 1
                                nodename = tobe_extlabel + str(numb)
                    ...
                    # delete real GT node
                    tree_stop.remove_node("ext0")
                    # convert to list
                    all_candidate = []
                    paths_leaves = [path_[1:] for path_ in tree_stop.paths_to_leaves()]
                    for onepath in paths_leaves:
                        onepath_data = []
                        for onepos in onepath:
                            onepath_data.append(tree_stop.get_node(onepos).data)
                        all_candidate.append(onepath_data)
...
def parse_args_():
    ...
    parser.add_argument(
        "--csvpath",
        type=str,
        default="./dataset/tracks10/origin/test_2024_04_08__14_44_25.csv",
    )
    parser.add_argument(
        "--savefolder",
        type=str,
        default="./dataset/tracks10/trainval_data_balanceflag",
    )

    parser.add_argument("--trainvalsplit_ratio", type=float, default=0.7)
    parser.add_argument("--add_stop_ratio", type=float, default=1 / 25.00)
...
if __name__ == "__main__":
    ...
    outputfolder = os.path.join(
        savefolder, f"past{opt.past_length}_depth{opt.tree_depth}_near{opt.node_number}"
    )
