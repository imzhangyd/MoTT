...
class Pred_clshead(nn.Module):
    def __init__(self, d_model, dropout=0.1, inoutdim=3, cand_num=25):
        ...
        self.cls_FFN = nn.Sequential(
            ...
            nn.Linear(d_model, d_model // 2, bias=True),
            nn.Linear(d_model // 2, cand_num, bias=True),
        )
    ...
    def forward(self, x):  # bs,globaltoken=1, d_model

        cls_h = self.cls_FFN(x).squeeze(dim=-2)
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        self.pred_clshead = Pred_clshead(
            d_model=self.d_model, dropout=dropout, inoutdim=inoutdim, cand_num=n_candi
        )
        self.pred_reghead = Pred_reghead(d_model=self.d_model, inoutdim=inoutdim)
    ...
    def forward(self, src_seq, trg_seq):
        ...
        trg_seq = torch.cat((pred_shift.unsqueeze(1), trg_seq), dim=1)

        enc_output, *_ = self.encoder_2(trg_seq, None, return_attns=True)
        # enc_output  [bs, len_cand+1, dim]

        pred_score = self.pred_clshead(enc_output[:, :1])
